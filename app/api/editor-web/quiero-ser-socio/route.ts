...
async function ensurePage() {
  ...
  const factoryContent = getFactoryContent();

  const { data: inserted, error: insertError } = await supabase
    .from("web_pages")
    .insert({
      slug: PAGE_SLUG,
      draft_content: factoryContent,
      published_content: factoryContent,
...
export async function GET() {
  try {
    const data = await ensurePage();

    return NextResponse.json({
      slug: data.slug,
      draftContent: data.draft_content || getFactoryContent(),
      publishedContent: data.published_content || getFactoryContent(),
...
export async function POST(request: NextRequest) {
  try {
    const body = (await request.json().catch(() => null)) as {
      action?: "save-draft" | "publish" | "restore" | "factory";
      content?: unknown;
    } | null;
    ...
    if (body.action === "factory") {
      ...
      const { error } = await supabase
        .from("web_pages")
        .update({ draft_content: factoryContent, updated_at: now })
        .eq("slug", PAGE_SLUG);

      if (error) throw error;

      return NextResponse.json({ ok: true, content: factoryContent });
    }
    ...
    if (body.action === "restore") {
      ...
      const restoredContent = data?.published_content || getFactoryContent();

      const { error: updateError } = await supabase
        .from("web_pages")
        .update({ draft_content: restoredContent, updated_at: now })
        .eq("slug", PAGE_SLUG);
    ...
    if (body.action === "save-draft") {
      const { error } = await supabase
        .from("web_pages")
        .update({ draft_content: body.content, updated_at: now })
        .eq("slug", PAGE_SLUG);
